...
let check_test f =
  match check_parse f with
    ...
    | `Pass expr ->
      ...
      let expected = input_line expected_file in
        try let result = string_of_expr (Interpreter.eval expr []) in
          if result <> expected
          then `Fail (`Eval, f, "  result: " ^ result ^ "\nexpected: " ^ expected)
          else `Pass (f, result)
        with
          | InterpreterError msg -> `Fail (`Eval, f, "evaluator threw InterpreterError: " ^ msg ^ "\n")

(* pretty-prints results *)
let print_result result =
  match result with
    | `Pass (f, _) -> print_string (f ^ "...pass\n")
    | `Fail (c, f, r) -> print_string (f ^ "..." ^ 
        (match c with
          | `Parse -> "parse failed:\n"
          | `Eval  -> "eval failed:\n")
        ^ r ^ "\n")
